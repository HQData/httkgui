  ...
  results_mc <- eventReactive(input$run, {
      if(input$output_type == "mc") {
          ...
          withProgress(message = "Calculation in progress", 
                       ...
                       for(i in 1:input$nSimulations) { #VERY SLOW - full of redundancies!
                          incProgress(1/input$nSimulations)
                          inits <- do.call(parameterize_pbtk, param_list)
                          
                          #
                          # Here we just replace inits with appropriate (MC) values
                          # Recalculate inits if we want to include KTS / FR / Clint (with variability)
                          #
                          if(exists("custom_param_values") && nrow(custom_param_values) > 0) {
                            which_are_inits <- custom_param_values$parameter %in% names(parameter_names)
                            which_are_additional <- custom_param_values$parameter %in% names(additional_parameters)
                            ...
                            if(any(which_are_additional)) {
                              torep <- apply(
                                custom_param_values[which_are_additional,c("mc.cv", "value")],
                                1,
                                function(x) {
                                  # browser()
                                  xsd <- sqrt(log(x[["mc.cv"]]^2 + 1))
                                  rlnorm(1, log(x[["value"]]) - (xsd^2)/2, xsd)
                                }
                              )
                            ...
                            torep <- apply(
                              custom_param_values[which_are_inits,c("mc.cv", "value")],
                              1,
                              function(x) {
                                # browser()
                                xsd <- sqrt(log(x[["mc.cv"]]^2 + 1))
